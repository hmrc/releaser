...
class CoordinatorSpecs extends WordSpec with Matchers with OptionValues with TryValues {
  ...
  "the coordinator" should {
    ...
    "extract the MetaData from the JSON grabbed from a repo" in {
      ...
      when(mockedBintrayHttp.get("https://bintray.com/api/v1/packages/hmrc/release-candidates/time-to-pay-arrangement"))
    ...
    "check the system_id matches the artefact name when search through arefacts in a repository" in {
      val mockedBintrayHttp = Mockito.mock(classOf[BintrayHttp])
      val artefactName: ArtefactName = "time-to-pay-arrangement"
      ...
      val repoFinder = new Repositories(metaDataGetter)(Seq(mavenRepository, ivyRepository, gradleRepository)).findReposOfArtefact(artefactName)

      repoFinder.get.scalaVersion shouldBe ""
